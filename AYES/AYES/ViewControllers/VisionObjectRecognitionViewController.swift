...
class VisionObjectRecognitionViewController: ViewController {
    ...
    func setupVision() -> NSError? {
        ...
        guard let modelURL = Bundle.main.url(forResource: "AYES_classification", withExtension: "mlmodelc") else {
    ...
    func drawVisionRequestResults(_ results: [Any]) {
        ...
        for observation in results where observation is VNClassificationObservation {
            ...
            if confidence >= 0.75 {
                print("Object Found - ", objectObservation.identifier, objectObservation.confidence)
                let rect = CGRect(origin: CGPoint(x: 30, y: 100),
                                  size: CGSize(width: 90, height: 270)
                )
                let shapeLayer = self.createRoundedRectLayerWithBounds(rect)
                let textLayer = self.createTextSubLayerInBounds(rect,
                                                                identifier: objectObservation.identifier,
